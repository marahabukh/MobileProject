    ...
    async function sync() {
      try {
        const [h, c, b] = await Promise.allSettled([
          getHero(), getCategories(), getBestSellers(),
        ]);
        if (h.status === "fulfilled" && h.value) { saveHero(h.value); setHero(h.value); }
        if (c.status === "fulfilled" && c.value?.length) { saveCategories(c.value); setCategories(c.value); }
        if (b.status === "fulfilled" && b.value?.length) { saveBestSellers(b.value); setBestSellers(b.value); }
      } catch (e) {
